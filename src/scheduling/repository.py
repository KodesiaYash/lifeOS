...
class ScheduledJobRepository:
    ...
    async def deactivate(self, job_id: uuid.UUID) -> None:
        await self.session.execute(
            update(ScheduledJob).where(ScheduledJob.id == job_id).values(active=False)
        )

    async def record_run(
        self, job_id: uuid.UUID, success: bool, next_run_at=None
    ) -> None:
...
class BackgroundTaskRepository:
    ...
    async def update_status(
        self, task_id: uuid.UUID, status: str, **kwargs: object
    ) -> None:
