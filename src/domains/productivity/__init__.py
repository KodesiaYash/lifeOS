...
class ProductivityPlugin(DomainPlugin):
    ...
    def get_tools(self) -> list[ToolDeclaration]:
        return [
            ToolDeclaration(
                tool_id="productivity.create_task",
                name="Create Task",
            ...
            ToolDeclaration(
                tool_id="productivity.complete_task",
                name="Complete Task",
            ...
            ToolDeclaration(
                tool_id="productivity.list_tasks",
                name="List Tasks",
            ...
            ToolDeclaration(
                tool_id="productivity.log_habit",
                name="Log Habit",
            ...
            ToolDeclaration(
                tool_id="productivity.get_daily_summary",
    ...
    def get_agents(self) -> list[AgentDeclaration]:
        return [
            AgentDeclaration(
                agent_type="productivity.planner",
                name="Planner",
            ...
            AgentDeclaration(
                agent_type="productivity.focus_coach",
                name="Focus Coach",
    ...
    def get_event_handlers(self) -> list[EventHandlerDeclaration]:
        return [
            EventHandlerDeclaration(
                event_pattern="productivity.task_created",
                handler=_on_task_created,
                description="Update project task count on new task.",
            ),
            EventHandlerDeclaration(
                event_pattern="productivity.task_completed",
                handler=_on_task_completed,
    ...
    def get_memory_categories(self) -> list[MemoryCategoryDeclaration]:
        return [
            MemoryCategoryDeclaration(
                ...
                description="User's work patterns and preferences",
                example_keys=["peak_hours", "work_style"],
            ),
            MemoryCategoryDeclaration(
                category="productivity_goal", description="Productivity targets", example_keys=["goal_name", "target"]
            ),
            MemoryCategoryDeclaration(
            ...
            MemoryCategoryDeclaration(
                category="habit_definition",
                description="Defined habits to track",
                example_keys=["habit_name", "frequency"],
            ),
            MemoryCategoryDeclaration(
                category="priority_rule", description="Prioritisation rules", example_keys=["rule_name", "condition"]
            ),
        ]
