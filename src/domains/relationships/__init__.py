...
class RelationshipsPlugin(DomainPlugin):
    ...
    def get_tools(self) -> list[ToolDeclaration]:
        return [
            ToolDeclaration(
                tool_id="relationships.log_interaction", name="Log Interaction",
                description="Log an interaction with a contact (call, meeting, message).",
                handler=_log_interaction,
                parameters_schema={"type": "object", "properties": {
                    "contact_name": {"type": "string"}, "interaction_type": {"type": "string"},
                    "description": {"type": "string"},
                }, "required": ["contact_name"]},
            ),
            ToolDeclaration(
                tool_id="relationships.create_contact", name="Create Contact",
                description="Create a new contact with name, relationship type, and notes.",
                handler=_create_contact,
                parameters_schema={"type": "object", "properties": {
                    "name": {"type": "string"}, "relationship_type": {"type": "string"},
                    "notes": {"type": "string"},
                }, "required": ["name"]},
            ),
            ToolDeclaration(
                tool_id="relationships.get_contact", name="Get Contact",
            ...
            ToolDeclaration(
                tool_id="relationships.schedule_event", name="Schedule Event",
                description="Schedule a social event or reminder for a contact.",
                handler=_schedule_event,
                parameters_schema={"type": "object", "properties": {
                    "contact_name": {"type": "string"}, "event_type": {"type": "string"},
                    "date": {"type": "string"},
    ...
    def get_agents(self) -> list[AgentDeclaration]:
        return [
            AgentDeclaration(
                agent_type="relationships.social_advisor", name="Social Advisor",
    ...
    def get_event_handlers(self) -> list[EventHandlerDeclaration]:
        return [
            EventHandlerDeclaration(event_pattern="relationships.interaction_logged", handler=_on_interaction_logged,
                                    description="Update contact last-interaction timestamp."),
            EventHandlerDeclaration(event_pattern="relationships.reminder_due", handler=_on_reminder_due,
                                    description="Send notification for relationship reminder."),
        ]

    def get_memory_categories(self) -> list[MemoryCategoryDeclaration]:
        return [
            MemoryCategoryDeclaration(category="contact_detail", description="Contact details and metadata",
                                      example_keys=["name", "relationship_type", "workplace"]),
            MemoryCategoryDeclaration(category="relationship_note", description="Notes about relationships",
                                      example_keys=["topic", "context"]),
            MemoryCategoryDeclaration(category="social_preference", description="Social preferences",
                                      example_keys=["communication_style", "meeting_preference"]),
            MemoryCategoryDeclaration(category="important_date", description="Birthdays, anniversaries, etc.",
