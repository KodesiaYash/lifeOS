...
class HomePlugin(DomainPlugin):
    ...
    def get_tools(self) -> list[ToolDeclaration]:
        return [
            ToolDeclaration(
                tool_id="home.create_task", name="Create Task",
            ...
            ToolDeclaration(
                tool_id="home.complete_task", name="Complete Task",
            ...
            ToolDeclaration(
                tool_id="home.add_to_shopping_list", name="Add to Shopping List",
            ...
            ToolDeclaration(
                tool_id="home.get_maintenance_schedule", name="Get Maintenance Schedule",
    ...
    def get_agents(self) -> list[AgentDeclaration]:
        return [
            AgentDeclaration(
                agent_type="home.household_manager", name="Household Manager",
                ...
                tools=["home.create_task", "home.complete_task", "home.add_to_shopping_list",
    ...
    def get_event_handlers(self) -> list[EventHandlerDeclaration]:
        return [
            EventHandlerDeclaration(event_pattern="home.task_created", handler=_on_task_created,
                                    description="Update household task dashboard."),
            EventHandlerDeclaration(event_pattern="home.maintenance_due", handler=_on_maintenance_due,
                                    description="Send maintenance reminder notification."),
        ]

    def get_memory_categories(self) -> list[MemoryCategoryDeclaration]:
        return [
            MemoryCategoryDeclaration(category="home_layout", description="Home layout and room info",
                                      example_keys=["room", "area_sqft"]),
            MemoryCategoryDeclaration(category="appliance_info", description="Appliance details and warranties",
                                      example_keys=["appliance", "brand", "warranty_end"]),
            MemoryCategoryDeclaration(category="maintenance_schedule", description="Recurring maintenance tasks",
                                      example_keys=["task", "frequency", "last_done"]),
            MemoryCategoryDeclaration(category="shopping_preference", description="Shopping preferences and stores",
                                      example_keys=["preferred_store", "brand_preference"]),
            MemoryCategoryDeclaration(category="household_rule", description="Household rules and routines",
                                      example_keys=["rule", "schedule"]),
        ]
