...
class LearningPlugin(DomainPlugin):
    ...
    def get_tools(self) -> list[ToolDeclaration]:
        return [
            ToolDeclaration(
                tool_id="learning.add_resource", name="Add Resource",
    ...
    def get_agents(self) -> list[AgentDeclaration]:
        return [
            AgentDeclaration(
                agent_type="learning.tutor", name="AI Tutor",
            ...
            AgentDeclaration(
                agent_type="learning.study_planner", name="Study Planner",
    ...
    def get_event_handlers(self) -> list[EventHandlerDeclaration]:
        return [
            EventHandlerDeclaration(event_pattern="learning.resource_added", handler=_on_resource_added,
                                    description="Index new resource for knowledge retrieval."),
            EventHandlerDeclaration(event_pattern="learning.session_logged", handler=_on_session_logged,
                                    description="Update progress tracking on session completion."),
        ]

    def get_memory_categories(self) -> list[MemoryCategoryDeclaration]:
        return [
            MemoryCategoryDeclaration(category="learning_goal", description="Learning targets",
                                      example_keys=["goal_name", "target_date"]),
            MemoryCategoryDeclaration(category="skill_level", description="Self-assessed skill levels",
                                      example_keys=["skill", "level"]),
            MemoryCategoryDeclaration(category="learning_preference", description="Learning style preferences",
                                      example_keys=["preferred_format", "study_time"]),
            MemoryCategoryDeclaration(category="book_note", description="Notes from books and articles",
                                      example_keys=["title", "chapter", "key_insight"]),
            MemoryCategoryDeclaration(category="course_progress", description="Course completion progress",
